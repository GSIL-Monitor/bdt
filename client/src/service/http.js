...
Axios.interceptors.response.use(response => {
  ...
  if (response.config.url.indexOf('/user/login') > -1 && response.data.returnCode == 1) {
    Cookie.set('TF_TOKEN', response.data.returnObject.token);
  ...
  if (error.config.message != 'none') {
    if (error.message == 'Network Error') {
      Vue.prototype.$message({message: '网络异常，请稍后再试', showClose: true, type: 'error', duration: 3000})
    } else if (error.message.indexOf('timeout') > -1) {
      Vue.prototype.$message({message: '请求超时，请稍后重试', showClose: true, type: 'error', duration: 3000})
    } else {
      Vue.prototype.$message({message: error.message, showClose: true, type: 'error', duration: 3000})
    }
  }
  return Promise.reject(error)
})
